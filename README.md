...
##### `daemon_group`
Name of the group account that the daemon is to run as.  The default is `sigul`.
...
##### `gnupg_key_length`
Primary key length for newly created keys.  Defaults to `1024` (bits).

##### `gnupg_key_type`
Primary key type for newly created keys.  Defaults to `DSA`.

##### `gnupg_key_usage`
Key usage flags for newly created keys.  Defaults to `sign`.
